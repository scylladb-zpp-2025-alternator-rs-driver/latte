...
pub fn rune_value_to_alternator_attribute(v: Value) -> Result<AttributeValue, AlternatorError> {
    match v {
        ...
        Value::Byte(b) => Ok(AttributeValue::N(b.to_string())),

        Value::Char(c) => Ok(AttributeValue::S(c.to_string())),
        ...
        _ => Err(AlternatorError::new(AlternatorErrorKind::BadInput(
            format!("Unsupported type for Alternator attribute: {:?}", v),
        ))),
    }
}
